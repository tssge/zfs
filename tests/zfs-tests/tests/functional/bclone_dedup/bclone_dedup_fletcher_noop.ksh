...
# data was written with a different checksum algorithm than the source.
# The index entries (fletcher4) won't match the stream records (sha256)
# because the AVL comparator checks checksum type first.
#
...
claim="recv -B with algorithm mismatch degrades gracefully (0 hits)."
...
# Create destination INDEPENDENTLY with fletcher4 and different data.
# This populates the bdi index with fletcher4 entries.
...
# Index has fletcher4 entries from the old destination data.
# Stream has sha256 records from the source.
# AVL lookup: sha256 key vs fletcher4 entries → type mismatch → 0 hits.
log_must eval "zfs send -c $SRCFS@snap1 | zfs recv -BF $DSTFS"
...
if [[ "$hits" -ne 0 ]]; then
	log_fail "Expected 0 hits with fletcher4/sha256 mismatch, got $hits"
fi
